...
exports.paperAnalysisSchema = exports.predictedExamPaperSchema = void 0;
exports.analyzePastPaper = analyzePastPaper;
exports.generatePredictedPaper = generatePredictedPaper;
exports.generateMultiPaperPrediction = generateMultiPaperPrediction;
const aiClient_1 = require("./aiClient");
const zod_1 = require("zod");
// Zod validation schemas
const examQuestionPartSchema = zod_1.z.object({
    label: zod_1.z.string(),
    marks: zod_1.z.number().min(1),
    text: zod_1.z.string()
});
const examQuestionSchema = zod_1.z.object({
    number: zod_1.z.string(),
    marks: zod_1.z.number().min(1),
    text: zod_1.z.string(),
    subparts: zod_1.z.array(examQuestionPartSchema).optional(),
    note: zod_1.z.string().optional()
});
const examSectionSchema = zod_1.z.object({
    title: zod_1.z.string(),
    sectionMarks: zod_1.z.number().min(0),
    attemptRule: zod_1.z.string(),
    questions: zod_1.z.array(examQuestionSchema)
});
exports.predictedExamPaperSchema = zod_1.z.object({
    ...
    sections: zod_1.z.array(examSectionSchema),
    footerNote: zod_1.z.string()
});
// Zod schema for enriched paper analysis
exports.paperAnalysisSchema = zod_1.z.object({
    repeatedTopics: zod_1.z.array(zod_1.z.string()),
    examPatterns: zod_1.z.array(zod_1.z.string()),
    importantChapters: zod_1.z.array(zod_1.z.string()),
    highPriorityQuestions: zod_1.z.array(zod_1.z.string()),
    predictedTopics: zod_1.z.array(zod_1.z.string()),
    // Rich intelligence metrics
    confidenceScore: zod_1.z.number().min(0).max(100).optional().default(85),
    chapterWeightage: zod_1.z.array(zod_1.z.object({
        chapter: zod_1.z.string(),
        weightage: zod_1.z.number().min(0).max(100),
        importanceScore: zod_1.z.number().min(0).max(100)
    })).optional(),
    unitFrequency: zod_1.z.array(zod_1.z.object({
        unit: zod_1.z.string(),
    ...
    trendAnalysis: zod_1.z.array(zod_1.z.string()).optional()
});
...
function checkDuplicateQuestions(paper) {
    const allQuestions = [];
...
function getMarkDistributionRules(targetMarks) {
    if (targetMarks === 50) {
        return {
            total: 50,
...
async function analyzePastPaper(extractedText, subject, university) {
    const messages = [
        {
            role: "system",
...
${extractedText}`,
    ...
    try {
        ...
        return exports.paperAnalysisSchema.parse(parsed);
    }
    catch (err) {
...
async function callAIWithRetry(messages, options, zodSchema, fallbackBuilder, retries = 2) {
    ...
    while (attempt <= retries) {
        ...
        try {
            const response = await (0, aiClient_1.generateAI)(currentMessages, options);
            const parsed = (0, aiClient_1.parseJsonFromAI)(response);
    ...
    try {
        const rawResponse = await (0, aiClient_1.generateAI)(messages, options);
...
async function generatePredictedPaper(analysis, options) {
...
- Important chapters: ${analysis.importantChapters.join(", ")}`
    ...
    return callAIWithRetry(messages, { maxTokens: 3000, temperature: 0.45, preferFast: true }, exports.predictedExamPaperSchema, (rawText) => buildFallbackExamPaper(rawText, options, analysis));
}
async function generateMultiPaperPrediction(papers, options) {
    // Aggregate analyses
...
function buildFallbackExamPaper(raw, options, analysis) {
    ...
    let globalQNum = 1;
    for (const q of secAQuestions) {
        q.number = String(globalQNum++);
    }
    for (const q of secBQuestions) {
        q.number = String(globalQNum++);
    }
    for (const q of secCQuestions) {
        q.number = String(globalQNum++);
    }
    return {
