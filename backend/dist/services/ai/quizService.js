...
function normalizeQuizQuestion(question, index) {
    ...
    if (normalized.type === "mcq") {
        ...
        if (correctAnswerIndex == null && normalized.correctAnswer) {
            ...
            correctAnswerIndex = options.findIndex((opt) => opt.trim().toLowerCase() === target);
        ...
        if (correctAnswerIndex == null || correctAnswerIndex < 0 || correctAnswerIndex >= options.length) {
            throw new Error(`Quiz question ${normalized.id} is missing a valid correctAnswerIndex for MCQ validation.`);
        }
...
async function generateQuiz(input) {
    ...
    const weakFocus = input.weakTopics?.length
        ...
        : "";
    const material = (0, textCleaner_1.truncateText)(input.content, QUIZ_CONTENT_LIMIT);
    const messages = [
        {
            role: "system",
...
${material}`,
    ...
    let questions;
    try {
        questions = (0, aiClient_1.parseJsonFromAI)(response);
    }
    catch {
