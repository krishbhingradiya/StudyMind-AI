...
class EmailService {
  ...
  constructor() {
    ...
    if (!env.brevoApiKey && !this.resend && !this.smtpTransporter) {
   ...
   */
  private async sendViaBrevo(email: string, subject: string, html: string, text?: string): Promise<boolean> {
    if (!env.brevoApiKey) return false;
    try {
      let senderName = "StudyMind AI";
      let senderEmail = "studymindai.admin@gmail.com";

      const fromMatch = env.smtpFrom.match(/^(.*?)\s*<(.*?)>$/);
      if (fromMatch) {
        senderName = fromMatch[1].trim().replace(/^['"]|['"]$/g, "");
        senderEmail = fromMatch[2].trim();
      } else if (env.smtpFrom.includes("@")) {
