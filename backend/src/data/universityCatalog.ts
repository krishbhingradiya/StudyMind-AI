export interface UniversitySeed {
  ...
  branches: string[];
  semesters: number;
  marking_scheme: { theory: number; practical: number; internal: number };
  exam_trends: string[];
  common_subjects: Record<string, string[]>;
}

export const UNIVERSITY_CATALOG: UniversitySeed[] = [
  {
    university_name: "GTU",
...
export function findUniversitySeed(name: string): UniversitySeed | undefined {
  const n = name.trim().toLowerCase();
  return UNIVERSITY_CATALOG.find(
    (u) =>
      u.university_name.toLowerCase() === n ||
